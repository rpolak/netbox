...
class RouteTargetBulkEditForm(
    ...
    pk = forms.ModelMultipleChoiceField(
        queryset=RouteTarget.objects.all(), widget=forms.MultipleHiddenInput()
    )
    tenant = DynamicModelChoiceField(queryset=Tenant.objects.all(), required=False)
...
class AggregateCSVForm(CustomFieldModelCSVForm):
    rir = CSVModelChoiceField(
        queryset=RIR.objects.all(), to_field_name="name", help_text="Assigned RIR"
    )
    tenant = CSVModelChoiceField(
        queryset=Tenant.objects.all(),
        required=False,
...
class AggregateFilterForm(BootstrapMixin, TenancyFilterForm, CustomFieldFilterForm):
    ...
    rir = DynamicModelMultipleChoiceField(
        queryset=RIR.objects.all(), to_field_name="slug", required=False, label="RIR"
    )
    tag = TagFilterField(model)
...
class PrefixCSVForm(CustomFieldModelCSVForm):
    ...
    status = CSVChoiceField(choices=PrefixStatusChoices, help_text="Operational status")
    ...
    def __init__(self, data=None, *args, **kwargs):
        ...
        if data:
            ...
            params = {
                ...
                f"group__{self.fields['vlan_group'].to_field_name}": data.get(
                    "vlan_group"
                ),
            }
...
class PrefixBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    ...
    region = DynamicModelChoiceField(
        queryset=Region.objects.all(), required=False, to_field_name="slug"
    )
    site = DynamicModelChoiceField(
        queryset=Site.objects.all(), required=False, query_params={"region": "$region"}
    )
    vrf = DynamicModelChoiceField(
        queryset=VRF.objects.all(),
        required=False,
    ...
    prefix_length = forms.IntegerField(
        min_value=PREFIX_LENGTH_MIN, max_value=PREFIX_LENGTH_MAX, required=False
    )
    tenant = DynamicModelChoiceField(queryset=Tenant.objects.all(), required=False)
...
class PrefixFilterForm(BootstrapMixin, TenancyFilterForm, CustomFieldFilterForm):
    ...
    present_in_vrf_id = DynamicModelChoiceField(
        queryset=VRF.objects.all(), required=False, label="Present in VRF"
    )
    status = forms.MultipleChoiceField(
        choices=PrefixStatusChoices, required=False, widget=StaticSelect2Multiple()
    )
...
class IPAddressForm(BootstrapMixin, TenancyForm, ReturnURLForm, CustomFieldModelForm):
        ...
        if instance:
            ...
            if instance.nat_inside:
                ...
                if type(nat_inside_parent) is Interface:
                    ...
                    initial[
                        "nat_virtual_machine"
                    ] = nat_inside_parent.virtual_machine.pk
    ...
    def clean(self):
        ...
        if self.cleaned_data.get("interface") and self.cleaned_data.get("vminterface"):
            raise forms.ValidationError(
                "Cannot select both a device interface and a virtual machine interface"
            )
        ...
        interface = self.cleaned_data.get("interface") or self.cleaned_data.get(
            "vminterface"
        )
    ...
    def save(self, *args, **kwargs):
        ...
        if interface and self.cleaned_data["primary_for_parent"]:
            ...
            interface
            and ipaddress.address.version == 4
            ...
            interface
            and ipaddress.address.version == 6
...
class IPAddressCSVForm(CustomFieldModelCSVForm):
    ...
    status = CSVChoiceField(
        choices=IPAddressStatusChoices, required=False, help_text="Operational status"
    )
    role = CSVChoiceField(
        choices=IPAddressRoleChoices, required=False, help_text="Functional role"
    )
    ...
    is_primary = forms.BooleanField(
        help_text="Make this the primary IP for the assigned device", required=False
    )
        ...
        if data:
            ...
            # Limit interface queryset by assigned device
                ...
                self.fields["interface"].queryset = VMInterface.objects.filter(
                    **{
                        f"virtual_machine__{self.fields['virtual_machine'].to_field_name}": data[
                            "virtual_machine"
                        ]
                    }
                )
...
class IPAddressAssignForm(BootstrapMixin, forms.Form):
    vrf_id = DynamicModelChoiceField(
        queryset=VRF.objects.all(), required=False, label="VRF", empty_label="Global"
    )
    q = forms.CharField(
        required=False,
        label="Search",
...
class IPAddressFilterForm(BootstrapMixin, TenancyFilterForm, CustomFieldFilterForm):
    ...
    status = forms.MultipleChoiceField(
        choices=IPAddressStatusChoices, required=False, widget=StaticSelect2Multiple()
    )
    role = forms.MultipleChoiceField(
        choices=IPAddressRoleChoices, required=False, widget=StaticSelect2Multiple()
    )
...
class VLANCSVForm(CustomFieldModelCSVForm):
    ...
    status = CSVChoiceField(choices=VLANStatusChoices, help_text="Operational status")
    role = CSVModelChoiceField(
        queryset=Role.objects.all(),
        required=False,
        to_field_name="name",
...
class VLANFilterForm(BootstrapMixin, TenancyFilterForm, CustomFieldFilterForm):
    ...
    status = forms.MultipleChoiceField(
        choices=VLANStatusChoices, required=False, widget=StaticSelect2Multiple()
    )
    role = DynamicModelMultipleChoiceField(
        queryset=Role.objects.all(),
        to_field_name="slug",
...
class ServiceForm(BootstrapMixin, CustomFieldModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.device:
            ...
            self.fields["ipaddresses"].queryset = IPAddress.objects.filter(
                vminterface__in=self.instance.virtual_machine.interfaces.values_list(
                    "id", flat=True
                )
            )
        else:
...
class ServiceBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=Service.objects.all(), widget=forms.MultipleHiddenInput()
    )
    protocol = forms.ChoiceField(
        choices=add_blank_choice(ServiceProtocolChoices),
        required=False,
