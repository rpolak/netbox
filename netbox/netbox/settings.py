...
DOCS_ROOT = getattr(
    configuration, "DOCS_ROOT", os.path.join(os.path.dirname(BASE_DIR), "docs")
)
...
MEDIA_ROOT = getattr(
    configuration, "MEDIA_ROOT", os.path.join(BASE_DIR, "media")
).rstrip("/")
...
RACK_ELEVATION_DEFAULT_UNIT_HEIGHT = getattr(
    configuration, "RACK_ELEVATION_DEFAULT_UNIT_HEIGHT", 22
)
RACK_ELEVATION_DEFAULT_UNIT_WIDTH = getattr(
    configuration, "RACK_ELEVATION_DEFAULT_UNIT_WIDTH", 220
)
REMOTE_AUTH_AUTO_CREATE_USER = getattr(
    configuration, "REMOTE_AUTH_AUTO_CREATE_USER", False
)
REMOTE_AUTH_BACKEND = getattr(
    configuration, "REMOTE_AUTH_BACKEND", "netbox.authentication.RemoteUserBackend"
)
...
REPORTS_ROOT = getattr(
    configuration, "REPORTS_ROOT", os.path.join(BASE_DIR, "reports")
).rstrip("/")
RQ_DEFAULT_TIMEOUT = getattr(configuration, "RQ_DEFAULT_TIMEOUT", 300)
SCRIPTS_ROOT = getattr(
    configuration, "SCRIPTS_ROOT", os.path.join(BASE_DIR, "scripts")
).rstrip("/")
...
if RELEASE_CHECK_TIMEOUT < 3600:
    raise ImproperlyConfigured(
        "RELEASE_CHECK_TIMEOUT has to be at least 3600 seconds (1 hour)"
    )
...
if type(REMOTE_AUTH_DEFAULT_PERMISSIONS) is not dict:
    try:
        REMOTE_AUTH_DEFAULT_PERMISSIONS = {
            perm: None for perm in REMOTE_AUTH_DEFAULT_PERMISSIONS
        }
        warnings.warn(
        ...
        raise ImproperlyConfigured(
            "REMOTE_AUTH_DEFAULT_PERMISSIONS must be a dictionary."
        )
...
if CACHING_REDIS_USING_SENTINEL:
    ...
    if CACHING_REDIS_PASSWORD:
        REDIS_CACHE_CON_STRING = "{}:{}@".format(
            REDIS_CACHE_CON_STRING, CACHING_REDIS_PASSWORD
        )
...
for plugin_name in PLUGINS:
    ...
    try:
        ...
        INSTALLED_APPS.append(
            "{}.{}".format(plugin_config.__module__, plugin_config.__name__)
        )
    ...
    CACHEOPS.update(
        {
            ...
            for key, value in plugin_config.caching_config.items()
        }
    )
