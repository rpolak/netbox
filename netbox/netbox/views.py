        ...
        (
            "devicetype",
            {
                "queryset": DeviceType.objects.prefetch_related(
                    "manufacturer"
                ).annotate(instance_count=get_subquery(Device, "device_type")),
...
class HomeView(View):
    ...
    def get(self, request):
        ...
        connected_interfaces = Interface.objects.restrict(request.user, "view").filter(
            _connected_interface__isnull=False, pk__lt=F("_connected_interface")
        )
        ...
        stats = {
            ...
            "devicetype_count": DeviceType.objects.restrict(
                request.user, "view"
            ).count(),
            ...
            "powerpanel_count": PowerPanel.objects.restrict(
                request.user, "view"
            ).count(),
                    ...
                    results.append(
                        {
                            ...
                            "table": table,
                            "url": "{}?q={}".format(
                                reverse(url), form.cleaned_data["q"]
                            ),
                        }
                    )
...
class StaticMediaFailureView(View):
    ...
    def get(self, request):
        return render(
            request, "media_failure.html", {"filename": request.GET.get("filename")}
        )
