    ...
    def test_status(self):
        params = {
            "status": [
                ...
                SiteStatusChoices.STATUS_PLANNED,
            ]
        }
        self.assertEqual(self.filterset(params, self.queryset).qs.count(), 2)
...
class RackGroupTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        parent_rack_groups = (
            RackGroup(
                name="Parent Rack Group 1", slug="parent-rack-group-1", site=sites[0]
            ),
            RackGroup(
                name="Parent Rack Group 2", slug="parent-rack-group-2", site=sites[1]
            ),
            RackGroup(
                name="Parent Rack Group 3", slug="parent-rack-group-3", site=sites[2]
            ),
        )
...
class RackTestCase(TestCase):
    ...
    def test_status(self):
        params = {
            "status": [
                RackStatusChoices.STATUS_ACTIVE,
                RackStatusChoices.STATUS_PLANNED,
            ]
        }
        self.assertEqual(self.filterset(params, self.queryset).qs.count(), 2)
...
class RackReservationTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        reservations = (
            RackReservation(
                rack=racks[0], units=[1, 2, 3], user=users[0], tenant=tenants[0]
            ),
            RackReservation(
                rack=racks[1], units=[4, 5, 6], user=users[1], tenant=tenants[1]
            ),
            RackReservation(
                rack=racks[2], units=[7, 8, 9], user=users[2], tenant=tenants[2]
            ),
        )
...
class DeviceTypeTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        ConsoleServerPortTemplate.objects.bulk_create(
            (
                ConsoleServerPortTemplate(
                    device_type=device_types[0], name="Console Server Port 1"
                ),
                ConsoleServerPortTemplate(
                    device_type=device_types[1], name="Console Server Port 2"
                ),
            )
        )
    ...
    def setUpTestData(cls):
        ...
        ConsoleServerPortTemplate.objects.bulk_create(
            (
                ...
                ConsoleServerPortTemplate(
                    device_type=device_types[2], name="Console Server Port 3"
                ),
            )
        )
...
class DeviceRoleTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        device_roles = (
            DeviceRole(
                name="Device Role 1", slug="device-role-1", color="ff0000", vm_role=True
            ),
            DeviceRole(
                name="Device Role 2", slug="device-role-2", color="00ff00", vm_role=True
            ),
            DeviceRole(
                name="Device Role 3",
                slug="device-role-3",
...
class DeviceTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        interfaces = (
            Interface(
                device=devices[0], name="Interface 1", mac_address="00-00-00-00-00-01"
            ),
            Interface(
                device=devices[1], name="Interface 2", mac_address="00-00-00-00-00-02"
            ),
        )
        Interface.objects.bulk_create(interfaces)
        rear_ports = (
            RearPort(
                device=devices[0], name="Rear Port 1", type=PortTypeChoices.TYPE_8P8C
            ),
            RearPort(
                device=devices[1], name="Rear Port 2", type=PortTypeChoices.TYPE_8P8C
            ),
        )
        ...
        Device.objects.filter(pk=devices[0].pk).update(
            virtual_chassis=virtual_chassis, vc_position=1, vc_priority=1
        )
        Device.objects.filter(pk=devices[1].pk).update(
            virtual_chassis=virtual_chassis, vc_position=2, vc_priority=2
        )
...
class ConsolePortTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # Cables
        Cable(
            termination_a=console_ports[0], termination_b=console_server_ports[0]
        ).save()
        Cable(
            termination_a=console_ports[1], termination_b=console_server_ports[1]
        ).save()
...
class ConsoleServerPortTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        console_server_ports = (
            ConsoleServerPort(
                device=devices[0], name="Console Server Port 1", description="First"
            ),
            ConsoleServerPort(
                device=devices[1], name="Console Server Port 2", description="Second"
            ),
            ConsoleServerPort(
                device=devices[2], name="Console Server Port 3", description="Third"
            ),
        )
        ConsoleServerPort.objects.bulk_create(console_server_ports)
        ...
        # Cables
        Cable(
            termination_a=console_server_ports[0], termination_b=console_ports[0]
        ).save()
        Cable(
            termination_a=console_server_ports[1], termination_b=console_ports[1]
        ).save()
        # Third port is not connected
...
class InventoryItemTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        device_type = DeviceType.objects.create(
            manufacturer=manufacturers[0], model="Model 1", slug="model-1"
        )
        device_role = DeviceRole.objects.create(
            name="Device Role 1", slug="device-role-1"
        )
        ...
        child_inventory_items = (
            InventoryItem(
                device=devices[0], name="Inventory Item 1A", parent=inventory_items[0]
            ),
            InventoryItem(
                device=devices[1], name="Inventory Item 2A", parent=inventory_items[1]
            ),
            InventoryItem(
...
class VirtualChassisTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        Device.objects.filter(pk=devices[1].pk).update(
            virtual_chassis=virtual_chassis[0]
        )
        Device.objects.filter(pk=devices[3].pk).update(
            virtual_chassis=virtual_chassis[1]
        )
        Device.objects.filter(pk=devices[5].pk).update(
            virtual_chassis=virtual_chassis[2]
        )
