...
class RegionListView(generic.ObjectListView):
    queryset = Region.objects.add_related_count(
        Region.objects.all(), Site, "region", "site_count", cumulative=True
    )
...
class SiteView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        stats = {
            ...
            .filter(terminations__site=instance)
            .count(),
            "vm_count": VirtualMachine.objects.restrict(request.user, "view")
            .filter(cluster__site=instance)
            .count(),
        }
        rack_groups = (
            RackGroup.objects.add_related_count(
...
class RackView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        nonracked_devices = Device.objects.filter(
            rack=instance, position__isnull=True
        ).prefetch_related("device_type__manufacturer")

        peer_racks = Rack.objects.restrict(request.user, "view").filter(
            site=instance.site
        )
        ...
        reservations = RackReservation.objects.restrict(request.user, "view").filter(
            rack=instance
        )
        power_feeds = (
            PowerFeed.objects.restrict(request.user, "view")
            .filter(rack=instance)
            .prefetch_related("power_panel")
        )

        device_count = (
            Device.objects.restrict(request.user, "view").filter(rack=instance).count()
        )
...
class DeviceTypeView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        instance_count = (
            Device.objects.restrict(request.user).filter(device_type=instance).count()
        )

        # Component tables
        consoleport_table = tables.ConsolePortTemplateTable(
            ConsolePortTemplate.objects.restrict(request.user, "view").filter(
                device_type=instance
            ),
            orderable=False,
        )
        consoleserverport_table = tables.ConsoleServerPortTemplateTable(
            ConsoleServerPortTemplate.objects.restrict(request.user, "view").filter(
                device_type=instance
            ),
            orderable=False,
        )
        powerport_table = tables.PowerPortTemplateTable(
            PowerPortTemplate.objects.restrict(request.user, "view").filter(
                device_type=instance
            ),
            orderable=False,
        )
        poweroutlet_table = tables.PowerOutletTemplateTable(
            PowerOutletTemplate.objects.restrict(request.user, "view").filter(
                device_type=instance
            ),
            orderable=False,
        )
        interface_table = tables.InterfaceTemplateTable(
            list(
                InterfaceTemplate.objects.restrict(request.user, "view").filter(
                    device_type=instance
                )
            ),
            orderable=False,
        ...
        front_port_table = tables.FrontPortTemplateTable(
            FrontPortTemplate.objects.restrict(request.user, "view").filter(
                device_type=instance
            ),
            orderable=False,
        )
        rear_port_table = tables.RearPortTemplateTable(
            RearPortTemplate.objects.restrict(request.user, "view").filter(
                device_type=instance
            ),
            orderable=False,
        )
        devicebay_table = tables.DeviceBayTemplateTable(
            DeviceBayTemplate.objects.restrict(request.user, "view").filter(
                device_type=instance
            ),
            orderable=False,
        )
...
class DeviceView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        related_devices = (
            Device.objects.restrict(request.user, "view")
            .filter(site=instance.site, device_role=instance.device_role)
            .exclude(pk=instance.pk)
            .prefetch_related("rack", "device_type__manufacturer")[:10]
        )
...
class DeviceConsolePortsView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        consoleport_table = tables.DeviceConsolePortTable(
            data=consoleports, user=request.user, orderable=False
        )
        if request.user.has_perm("dcim.change_consoleport") or request.user.has_perm(
...
class DeviceConsoleServerPortsView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        ) or request.user.has_perm("dcim.delete_consoleserverport"):
            consoleserverport_table.columns.show("pk")
...
class DevicePowerPortsView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        powerport_table = tables.DevicePowerPortTable(
            data=powerports, user=request.user, orderable=False
        )
        if request.user.has_perm("dcim.change_powerport") or request.user.has_perm(
...
class DevicePowerOutletsView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        poweroutlet_table = tables.DevicePowerOutletTable(
            data=poweroutlets, user=request.user, orderable=False
        )
        if request.user.has_perm("dcim.change_poweroutlet") or request.user.has_perm(
...
class DeviceInterfacesView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        interfaces = instance.vc_interfaces.restrict(
            request.user, "view"
        ).prefetch_related(
        ...
        interface_table = tables.DeviceInterfaceTable(
            data=interfaces, user=request.user, orderable=False
        )
...
class DeviceFrontPortsView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        frontport_table = tables.DeviceFrontPortTable(
            data=frontports, user=request.user, orderable=False
        )
        if request.user.has_perm("dcim.change_frontport") or request.user.has_perm(
...
class DeviceRearPortsView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        rearports = (
            RearPort.objects.restrict(request.user, "view")
            .filter(device=instance)
            .prefetch_related("cable")
        )
        rearport_table = tables.DeviceRearPortTable(
            data=rearports, user=request.user, orderable=False
        )
        if request.user.has_perm("dcim.change_rearport") or request.user.has_perm(
...
class DeviceDeviceBaysView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        devicebay_table = tables.DeviceDeviceBayTable(
            data=devicebays, user=request.user, orderable=False
        )
        if request.user.has_perm("dcim.change_devicebay") or request.user.has_perm(
...
class DeviceInventoryView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        ...
        inventoryitem_table = tables.DeviceInventoryItemTable(
            data=inventoryitems, user=request.user, orderable=False
        )
        if request.user.has_perm("dcim.change_inventoryitem") or request.user.has_perm(
...
class CableCreateView(generic.ObjectEditView):
    ...
    def alter_obj(self, obj, request, url_args, url_kwargs):
        ...
        self.termination_b_type = ContentType.objects.get(
            model=termination_b_type_name.replace("-", "")
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "termination_b_rack" not in initial_data:
            initial_data["termination_b_rack"] = getattr(
                obj.termination_a.parent, "rack", None
            )
...
class ConsoleConnectionsListView(generic.ObjectListView):
    ...
    def queryset_to_csv(self):
        ...
        for obj in self.queryset:
            csv = csv_format(
                [
                    obj._path.destination.device.identifier
                    if obj._path.destination
                    else None,
...
class VirtualChassisEditView(ObjectPermissionRequiredMixin, GetReturnURLMixin, View):
    ...
    def get(self, request, pk):
        ...
        members_queryset = virtual_chassis.members.prefetch_related("rack").order_by(
            "vc_position"
        )
...
class VirtualChassisAddMemberView(
    ObjectPermissionRequiredMixin, GetReturnURLMixin, View
):
    queryset = VirtualChassis.objects.all()
...
class VirtualChassisRemoveMemberView(
    ObjectPermissionRequiredMixin, GetReturnURLMixin, View
):
    queryset = Device.objects.all()
...
class PowerPanelView(generic.ObjectView):
    ...
    def get_extra_context(self, request, instance):
        power_feeds = (
            PowerFeed.objects.restrict(request.user)
            .filter(power_panel=instance)
            .prefetch_related("rack")
        )
