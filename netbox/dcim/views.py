...
class RackRoleListView(ObjectListView):
    queryset = RackRole.objects.annotate(rack_count=get_subquery(Rack, "role"))
...
class RackListView(ObjectListView):
    ...
    ).annotate(device_count=get_subquery(Device, "rack"))
...
class RackView(ObjectView):
    ...
    def get(self, request, pk):
        ...
        nonracked_devices = Device.objects.filter(
            rack=rack, position__isnull=True
...
class DeviceTypeView(ObjectView):
    ...
    def get(self, request, pk):
        ...
        instance_count = (
            Device.objects.restrict(request.user).filter(device_type=devicetype).count()
        )
    ...
    def get(self, request, pk):
        ...
        if device.virtual_chassis is not None:
            vc_members = (
                ...
                .filter(virtual_chassis=device.virtual_chassis)
        ...
        powerports = (
            ...
            .filter(device=device)
            .prefetch_related(
                "_connected_poweroutlet__device",
                "cable",
            )
        )
        ...
        # Interfaces
        interfaces = device.vc_interfaces.restrict(
            request.user, "view"
        ).prefetch_related(
            ...
            "cable",
            "cable__termination_a",
            "cable__termination_b",
            "tags",
        )
        ...
        related_devices = (
            Device.objects.restrict(request.user, "view")
            .filter(site=device.site, device_role=device.device_role)
        ...
        return render(
            ...
            "dcim/device.html",
            {
                ...
                "show_interface_graphs": Graph.objects.filter(
                    type__model="interface"
                ).exists(),
            },
        )
...
class DeviceInventoryView(ObjectView):
    ...
    def get(self, request, pk):
        ...
        inventory_items = (
            ...
            .filter(device=device, parent=None)
            .prefetch_related("manufacturer", "child_items")
        )

        return render(
            request,
            "dcim/device_inventory.html",
            {
                "device": device,
...
class DeviceLLDPNeighborsView(ObjectView):
    ...
    def get(self, request, pk):
        ...
        device = get_object_or_404(self.queryset, pk=pk)
        interfaces = (
            device.vc_interfaces.restrict(request.user, "view")
            .exclude(type__in=NONCONNECTABLE_IFACE_TYPES)
            .prefetch_related("_connected_interface__device")
        )

        return render(
            request,
            "dcim/device_lldp_neighbors.html",
            {
                "device": device,
                "interfaces": interfaces,
...
class InterfaceView(ObjectView):
    ...
    def get(self, request, pk):
        ...
        ipaddress_table = InterfaceIPAddressTable(
            data=interface.ip_addresses.restrict(request.user, "view").prefetch_related(
                "vrf", "tenant"
            ),
            orderable=False,
        )
        ...
        for vlan in interface.tagged_vlans.restrict(request.user).prefetch_related(
    ...
    def get(self, request, pk):
        ...
        return render(
            ...
            "dcim/devicebay_populate.html",
            {
                ...
                "form": form,
                "return_url": reverse(
                    "dcim:device", kwargs={"pk": device_bay.device.pk}
                ),
            },
        )
    ...
    def get(self, request, pk):
        ...
        total_length = sum(
            [
                entry[1]._abs_length
                for entry in path
                if entry[1] and entry[1]._abs_length
            ]
        )

        return render(
            request,
            "dcim/cable_trace.html",
            {
                "obj": obj,
                "trace": path,
...
class ConsoleConnectionsListView(ObjectListView):
    queryset = (
        ConsolePort.objects.prefetch_related("device", "connected_endpoint__device")
        .filter(connected_endpoint__isnull=False)
        .order_by(
    ...
    def queryset_to_csv(self):
        ...
        for obj in self.queryset:
            csv = csv_format(
                [
                    obj.connected_endpoint.device.identifier
...
class PowerConnectionsListView(ObjectListView):
    queryset = (
        ...
        .filter(_connected_poweroutlet__isnull=False)
        .order_by(
            "cable",
            "_connected_poweroutlet__device__name",
            "_connected_poweroutlet__name",
        )
    )
...
class VirtualChassisView(ObjectView):
    ...
    def get(self, request, pk):
        ...
        members = Device.objects.restrict(request.user).filter(
