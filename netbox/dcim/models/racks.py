    ...
    def clean(self):
        ...
        if self.parent and self.parent.site != self.site:
            raise ValidationError(
                f"Parent rack group ({self.parent}) must belong to the same site ({self.site})"
            )
...
class Rack(ChangeLoggedModel, CustomFieldModel):
    ...
    site = models.ForeignKey(
        to="dcim.Site", on_delete=models.PROTECT, related_name="racks"
    )
    ...
    type = models.CharField(
        choices=RackTypeChoices, max_length=50, blank=True, verbose_name="Type"
    )
    ...
    outer_width = models.PositiveSmallIntegerField(
        blank=True, null=True, help_text="Outer dimension of rack (width)"
    )
    outer_depth = models.PositiveSmallIntegerField(
        blank=True, null=True, help_text="Outer dimension of rack (depth)"
    )
    ...
    class Meta:
        ordering = (
            ...
            "_name",
            "pk",
        )  # (site, group, name) may be non-unique
        unique_together = (
    ...
    def clean(self):
        ...
        if (
            self.outer_width is not None or self.outer_depth is not None
        ) and not self.outer_unit:
            raise ValidationError(
                "Must specify a unit when setting an outer width/depth"
            )
        ...
        if self.pk:
            ...
            top_device = (
                Device.objects.filter(rack=self)
            ...
            if self.group:
                if self.group.site != self.site:
                    raise ValidationError(
                        {
                            "group": "Rack group must be from the same site, {}.".format(
                                self.site
                            )
                        }
                    )
    ...
    def get_rack_units(
        ...
        if self.pk:
            ...
            if user is not None:
                permitted_device_ids = self.devices.restrict(user, "view").values_list(
                    "pk", flat=True
                )
            ...
            for device in queryset:
                if expand_devices:
                    for u in range(
                        device.position, device.position + device.device_type.u_height
                    ):
    ...
    def get_elevation_svg(
        ...
        elevation = RackElevationSVG(
            self, user=user, include_images=include_images, base_url=base_url
        )
...
class RackReservation(ChangeLoggedModel, CustomFieldModel):
    ...
    rack = models.ForeignKey(
        to="dcim.Rack", on_delete=models.CASCADE, related_name="reservations"
    )
