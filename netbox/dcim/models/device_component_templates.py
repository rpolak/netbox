...
class ComponentTemplateModel(models.Model):
    device_type = models.ForeignKey(
        to="dcim.DeviceType", on_delete=models.CASCADE, related_name="%(class)ss"
    )
...
class ConsolePortTemplate(ComponentTemplateModel):
    ...
    def instantiate(self, device):
        return ConsolePort(
            device=device, name=self.name, label=self.label, type=self.type
        )
...
class PowerOutletTemplate(ComponentTemplateModel):
    ...
    def clean(self):
        ...
        if self.power_port and self.power_port.device_type != self.device_type:
            raise ValidationError(
                "Parent power port ({}) must belong to the same device type".format(
                    self.power_port
                )
            )
...
class DeviceBayTemplate(ComponentTemplateModel):
    ...
    def clean(self):
        if (
            self.device_type
            and self.device_type.subdevice_role != SubdeviceRoleChoices.ROLE_PARENT
        ):
            raise ValidationError(
