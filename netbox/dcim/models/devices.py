...
class Device(ChangeLoggedModel, ConfigContextModel, CustomFieldModel):
    ...
    device_type = models.ForeignKey(
        to="dcim.DeviceType", on_delete=models.PROTECT, related_name="instances"
    )
    device_role = models.ForeignKey(
        to="dcim.DeviceRole", on_delete=models.PROTECT, related_name="devices"
    )
    ...
    _name = NaturalOrderingField(
        target_field="name", max_length=100, blank=True, null=True
    )
    ...
    site = models.ForeignKey(
        to="dcim.Site", on_delete=models.PROTECT, related_name="devices"
    )
    ...
    face = models.CharField(
        max_length=50, blank=True, choices=DeviceFaceChoices, verbose_name="Rack face"
    )
    ...
    def clean(self):
        ...
        if self.rack and self.site != self.rack.site:
            raise ValidationError(
                {
                    "rack": "Rack {} does not belong to site {}.".format(
                        self.rack, self.site
                    ),
                }
            )
        ...
        if self.position and self.device_type.u_height == 0:
            raise ValidationError(
                {
                    "position": "A U0 device type ({}) cannot be assigned to a rack position.".format(
                        self.device_type
                    )
                }
            )
        ...
        if self.primary_ip4:
            ...
            if self.primary_ip4.assigned_object in vc_interfaces:
                ...
                and self.primary_ip4.nat_inside.assigned_object in vc_interfaces
            ):
                pass
            else:
                raise ValidationError(
        ...
        if self.primary_ip6:
            ...
            if self.primary_ip6.assigned_object in vc_interfaces:
                ...
                and self.primary_ip6.nat_inside.assigned_object in vc_interfaces
            ):
                pass
            else:
                raise ValidationError(
        ...
        if (
            self.cluster
            and self.cluster.site is not None
            and self.cluster.site != self.site
        ):
            raise ValidationError(
                {
                    "cluster": "The assigned cluster belongs to a different site ({})".format(
                        self.cluster.site
                    )
                }
            )
    ...
    def display_name(self):
        if self.name:
            ...
            return (
                f"{self.device_type.manufacturer} {self.device_type.model} ({self.pk})"
            )
        else:
...
class Cable(ChangeLoggedModel):
    ...
    termination_a = GenericForeignKey(
        ct_field="termination_a_type", fk_field="termination_a_id"
    )
    ...
    termination_b = GenericForeignKey(
        ct_field="termination_b_type", fk_field="termination_b_id"
    )
    ...
    def clean(self):
        ...
        try:
            ...
            raise ValidationError(
                {
                    "termination_a": "Invalid ID for type {}".format(
        ...
        try:
            ...
            raise ValidationError(
                {
                    "termination_b": "Invalid ID for type {}".format(
        ...
        if (
            type_a == "interface"
            and self.termination_a.type in NONCONNECTABLE_IFACE_TYPES
        ):
            raise ValidationError(
                {
        ...
        if (
            type_b == "interface"
            and self.termination_b.type in NONCONNECTABLE_IFACE_TYPES
        ):
            raise ValidationError(
                {
        ...
        for term_a, term_b in [
            ...
            if isinstance(term_a, RearPort) and term_a.positions > 1:
                ...
                if (
                    isinstance(term_b, RearPort)
                    and term_b.positions > 1
                    and term_a.positions != term_b.positions
                ):
                    raise ValidationError(
    ...
    def to_csv(self):
        return (
            "{}.{}".format(
                self.termination_a_type.app_label, self.termination_a_type.model
            ),
            self.termination_a_id,
            "{}.{}".format(
                self.termination_b_type.app_label, self.termination_b_type.model
            ),
