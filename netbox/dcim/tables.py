...
class RackGroupTable(BaseTable):
    ...
    site = tables.LinkColumn(
        viewname="dcim:site", args=[Accessor("site__slug")], verbose_name="Site"
    )
...
class RackTable(BaseTable):
    ...
    u_height = tables.TemplateColumn(
        template_code="{{ record.u_height }}U", verbose_name="Height"
    )
...
class RackDetailTable(RackTable):
    device_count = tables.TemplateColumn(
        template_code=RACK_DEVICE_COUNT, verbose_name="Devices"
    )
    get_utilization = tables.TemplateColumn(
        template_code=UTILIZATION_GRAPH, orderable=False, verbose_name="Space"
    )
...
class DeviceRoleTable(BaseTable):
    pk = ToggleColumn()
    device_count = tables.TemplateColumn(
        template_code=DEVICEROLE_DEVICE_COUNT, verbose_name="Devices"
    )
    vm_count = tables.TemplateColumn(
        template_code=DEVICEROLE_VM_COUNT, verbose_name="VMs"
    )
    color = tables.TemplateColumn(template_code=COLOR_LABEL, verbose_name="Label")
...
class PlatformTable(BaseTable):
    pk = ToggleColumn()
    device_count = tables.TemplateColumn(
        template_code=PLATFORM_DEVICE_COUNT, verbose_name="Devices"
    )
    vm_count = tables.TemplateColumn(
        template_code=PLATFORM_VM_COUNT, verbose_name="VMs"
    )
...
class DeviceTable(BaseTable):
    ...
    primary_ip = tables.TemplateColumn(
        template_code=DEVICE_PRIMARY_IP, orderable=False, verbose_name="IP Address"
    )
    ...
    cluster = tables.LinkColumn(
        viewname="virtualization:cluster", args=[Accessor("cluster__pk")]
    )
    virtual_chassis = tables.LinkColumn(
        viewname="dcim:virtualchassis", args=[Accessor("virtual_chassis__pk")]
    )
...
class ConsoleConnectionTable(BaseTable):
    ...
    connected_endpoint = tables.Column(linkify=True, verbose_name="Port")
    device = tables.Column(linkify=True)
    name = tables.Column(linkify=True, verbose_name="Console Port")
    connection_status = tables.TemplateColumn(
        template_code=CONNECTION_STATUS, verbose_name="Status"
    )
...
class PowerConnectionTable(BaseTable):
    ...
    outlet = tables.Column(
        accessor=Accessor("_connected_poweroutlet"), linkify=True, verbose_name="Outlet"
    )
    device = tables.Column(linkify=True)
...
class PowerFeedTable(BaseTable):
    ...
    rack = tables.Column(linkify=True)
    status = tables.TemplateColumn(template_code=STATUS_LABEL)
    type = tables.TemplateColumn(template_code=TYPE_LABEL)
