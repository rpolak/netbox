...
class InterfaceCommonForm(forms.Form):
    def clean(self):
        ...
        if (
            self.cleaned_data["mode"] == InterfaceModeChoices.MODE_ACCESS
            and tagged_vlans
        ):
...
class SiteCSVForm(CustomFieldModelCSVForm):
    status = CSVChoiceField(
        choices=SiteStatusChoices, required=False, help_text="Operational status"
    )
...
class SiteBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=Site.objects.all(), widget=forms.MultipleHiddenInput
    )
    ...
    asn = forms.IntegerField(
        min_value=BGP_ASN_MIN, max_value=BGP_ASN_MAX, required=False, label="ASN"
    )
...
class SiteFilterForm(BootstrapMixin, TenancyFilterForm, CustomFieldFilterForm):
    ...
    status = forms.MultipleChoiceField(
        choices=SiteStatusChoices, required=False, widget=StaticSelect2Multiple()
    )
    region = DynamicModelMultipleChoiceField(
        queryset=Region.objects.all(), to_field_name="slug", required=False
    )
...
class RackGroupForm(BootstrapMixin, forms.ModelForm):
    region = DynamicModelChoiceField(
        queryset=Region.objects.all(), required=False, initial_params={"sites": "$site"}
    )
    site = DynamicModelChoiceField(
        queryset=Site.objects.all(), query_params={"region_id": "$region"}
    )
...
class RackGroupCSVForm(CSVModelForm):
    site = CSVModelChoiceField(
        queryset=Site.objects.all(), to_field_name="name", help_text="Assigned site"
    )
    parent = CSVModelChoiceField(
        queryset=RackGroup.objects.all(),
        required=False,
...
class RackCSVForm(CustomFieldModelCSVForm):
    ...
    group = CSVModelChoiceField(
        queryset=RackGroup.objects.all(), required=False, to_field_name="name"
    )
    tenant = CSVModelChoiceField(
        queryset=Tenant.objects.all(),
        required=False,
    ...
    status = CSVChoiceField(
        choices=RackStatusChoices, required=False, help_text="Operational status"
    )
    ...
    type = CSVChoiceField(
        choices=RackTypeChoices, required=False, help_text="Rack type"
    )
    width = forms.ChoiceField(
        choices=RackWidthChoices, help_text="Rail-to-rail width (in inches)"
    )
...
class RackBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=Rack.objects.all(), widget=forms.MultipleHiddenInput
    )
    region = DynamicModelChoiceField(
        queryset=Region.objects.all(), required=False, initial_params={"sites": "$site"}
    )
    ...
    desc_units = forms.NullBooleanField(
        required=False, widget=BulkEditNullBooleanSelect, label="Descending units"
    )
...
class RackFilterForm(BootstrapMixin, TenancyFilterForm, CustomFieldFilterForm):
    ...
    status = forms.MultipleChoiceField(
        choices=RackStatusChoices, required=False, widget=StaticSelect2Multiple()
    )
    type = forms.MultipleChoiceField(
        choices=RackTypeChoices, required=False, widget=StaticSelect2Multiple()
    )
    width = forms.MultipleChoiceField(
        choices=RackWidthChoices, required=False, widget=StaticSelect2Multiple()
    )
...
class RackReservationForm(BootstrapMixin, TenancyForm, CustomFieldModelForm):
    ...
    user = forms.ModelChoiceField(
        queryset=User.objects.order_by("username"), widget=StaticSelect2()
    )
    tags = DynamicModelMultipleChoiceField(queryset=Tag.objects.all(), required=False)
...
class RackReservationCSVForm(CustomFieldModelCSVForm):
    site = CSVModelChoiceField(
        queryset=Site.objects.all(), to_field_name="name", help_text="Parent site"
    )
    ...
    rack = CSVModelChoiceField(
        queryset=Rack.objects.all(), to_field_name="name", help_text="Rack"
    )
    ...
    def __init__(self, data=None, *args, **kwargs):
        ...
        if data:
            ...
            params = {f"site__{self.fields['site'].to_field_name}": data.get("site")}
            self.fields["rack_group"].queryset = self.fields[
                "rack_group"
            ...
            params = {
                ...
                f"group__{self.fields['rack_group'].to_field_name}": data.get(
                    "rack_group"
                ),
            }
...
class RackReservationBulkEditForm(
    BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm
):
    pk = forms.ModelMultipleChoiceField(
        queryset=RackReservation.objects.all(), widget=forms.MultipleHiddenInput()
    )
    user = forms.ModelChoiceField(
...
class DeviceTypeBulkEditForm(
    BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm
):
    pk = forms.ModelMultipleChoiceField(
        queryset=DeviceType.objects.all(), widget=forms.MultipleHiddenInput()
    )
    ...
    is_full_depth = forms.NullBooleanField(
        required=False, widget=BulkEditNullBooleanSelect(), label="Is full depth"
    )
...
class ConsolePortTemplateCreateForm(ComponentTemplateCreateForm):
    type = forms.ChoiceField(
        choices=add_blank_choice(ConsolePortTypeChoices), widget=StaticSelect2()
    )
    field_order = (
        "manufacturer",
        "device_type",
...
class ConsolePortTemplateBulkEditForm(BootstrapMixin, BulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=ConsolePortTemplate.objects.all(), widget=forms.MultipleHiddenInput()
    )
...
class PowerPortTemplateCreateForm(ComponentTemplateCreateForm):
    type = forms.ChoiceField(
        choices=add_blank_choice(PowerPortTypeChoices), required=False
    )
    maximum_draw = forms.IntegerField(
        min_value=1, required=False, help_text="Maximum power draw (watts)"
    )
...
class PowerPortTemplateBulkEditForm(BootstrapMixin, BulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=PowerPortTemplate.objects.all(), widget=forms.MultipleHiddenInput()
    )
    label = forms.CharField(max_length=64, required=False)
    type = forms.ChoiceField(
...
class PowerOutletTemplateCreateForm(ComponentTemplateCreateForm):
    type = forms.ChoiceField(
        choices=add_blank_choice(PowerOutletTypeChoices), required=False
    )
...
class PowerOutletTemplateBulkEditForm(BootstrapMixin, BulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=PowerOutletTemplate.objects.all(), widget=forms.MultipleHiddenInput()
    )
...
class InterfaceTemplateBulkEditForm(BootstrapMixin, BulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=InterfaceTemplate.objects.all(), widget=forms.MultipleHiddenInput()
    )
    label = forms.CharField(max_length=64, required=False)
    type = forms.ChoiceField(
    ...
    mgmt_only = forms.NullBooleanField(
        required=False, widget=BulkEditNullBooleanSelect, label="Management only"
    )
    description = forms.CharField(required=False)
...
class FrontPortTemplateBulkEditForm(BootstrapMixin, BulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=FrontPortTemplate.objects.all(), widget=forms.MultipleHiddenInput()
    )
    label = forms.CharField(max_length=64, required=False)
    type = forms.ChoiceField(
...
class RearPortTemplateBulkEditForm(BootstrapMixin, BulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=RearPortTemplate.objects.all(), widget=forms.MultipleHiddenInput()
    )
    label = forms.CharField(max_length=64, required=False)
    type = forms.ChoiceField(
        choices=add_blank_choice(PortTypeChoices),
...
class DeviceBayTemplateBulkEditForm(BootstrapMixin, BulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=DeviceBayTemplate.objects.all(), widget=forms.MultipleHiddenInput()
    )
    label = forms.CharField(max_length=64, required=False)
    description = forms.CharField(required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            for family in [4, 6]:
                ...
                if interface_ips:
                    ip_list = [
                        (ip.id, f"{ip.address} ({ip.assigned_object})")
...
class BaseDeviceCSVForm(CustomFieldModelCSVForm):
    ...
    status = CSVChoiceField(choices=DeviceStatusChoices, help_text="Operational status")
    ...
    def __init__(self, data=None, *args, **kwargs):
        ...
        if data:
            ...
            params = {
                f"manufacturer__{self.fields['manufacturer'].to_field_name}": data.get(
                    "manufacturer"
                )
            }
...
class DeviceCSVForm(BaseDeviceCSVForm):
    ...
    face = CSVChoiceField(
        choices=DeviceFaceChoices, required=False, help_text="Mounted rack face"
    )
...
class ChildDeviceCSVForm(BaseDeviceCSVForm):
    parent = CSVModelChoiceField(
        queryset=Device.objects.all(), to_field_name="name", help_text="Parent device"
    )
    ...
    def __init__(self, data=None, *args, **kwargs):
        ...
        if data:
            ...
            params = {
                f"device__{self.fields['parent'].to_field_name}": data.get("parent")
            }
...
class DeviceFilterForm(
    ...
    status = forms.MultipleChoiceField(
        choices=DeviceStatusChoices, required=False, widget=StaticSelect2Multiple()
    )
...
class ComponentCreateForm(ComponentForm):
    ...
    device = DynamicModelChoiceField(
        queryset=Device.objects.all(), display_field="display_name"
    )
    description = forms.CharField(max_length=100, required=False)
    tags = DynamicModelMultipleChoiceField(queryset=Tag.objects.all(), required=False)
...
class ConsolePortFilterForm(DeviceComponentFilterForm):
    model = ConsolePort
    type = forms.MultipleChoiceField(
        choices=ConsolePortTypeChoices, required=False, widget=StaticSelect2Multiple()
    )
    tag = TagFilterField(model)
...
class ConsolePortBulkEditForm(
    ...
    pk = forms.ModelMultipleChoiceField(
        queryset=ConsolePort.objects.all(), widget=forms.MultipleHiddenInput()
    )
...
class ConsolePortCSVForm(CSVModelForm):
    ...
    type = CSVChoiceField(
        choices=ConsolePortTypeChoices, required=False, help_text="Port type"
    )
...
class ConsoleServerPortBulkCreateForm(
    form_from_model(ConsoleServerPort, ["type"]), DeviceBulkAddComponentForm
):
    field_order = ("name_pattern", "label_pattern", "type", "description", "tags")
...
class ConsoleServerPortBulkEditForm(
    ...
    pk = forms.ModelMultipleChoiceField(
        queryset=ConsoleServerPort.objects.all(), widget=forms.MultipleHiddenInput()
    )
...
class PowerPortFilterForm(DeviceComponentFilterForm):
    model = PowerPort
    type = forms.MultipleChoiceField(
        choices=PowerPortTypeChoices, required=False, widget=StaticSelect2Multiple()
    )
    tag = TagFilterField(model)
...
class PowerPortCreateForm(ComponentCreateForm):
    ...
    maximum_draw = forms.IntegerField(
        min_value=1, required=False, help_text="Maximum draw in watts"
    )
    allocated_draw = forms.IntegerField(
        min_value=1, required=False, help_text="Allocated draw in watts"
    )
    field_order = (
        "device",
        "name_pattern",
...
class PowerPortBulkEditForm(
    form_from_model(
        PowerPort, ["label", "type", "maximum_draw", "allocated_draw", "description"]
    ),
    BootstrapMixin,
    AddRemoveTagsForm,
    BulkEditForm,
    ...
    pk = forms.ModelMultipleChoiceField(
        queryset=PowerPort.objects.all(), widget=forms.MultipleHiddenInput()
    )
...
class PowerPortCSVForm(CSVModelForm):
    device = CSVModelChoiceField(queryset=Device.objects.all(), to_field_name="name")
    type = CSVChoiceField(
        choices=PowerPortTypeChoices, required=False, help_text="Port type"
    )
...
class PowerOutletFilterForm(DeviceComponentFilterForm):
    model = PowerOutlet
    type = forms.MultipleChoiceField(
        choices=PowerOutletTypeChoices, required=False, widget=StaticSelect2Multiple()
    )
    tag = TagFilterField(model)
...
class PowerOutletForm(BootstrapMixin, forms.ModelForm):
    power_port = forms.ModelChoiceField(
        queryset=PowerPort.objects.all(), required=False
    )
    tags = DynamicModelMultipleChoiceField(queryset=Tag.objects.all(), required=False)
...
class PowerOutletBulkCreateForm(
    form_from_model(PowerOutlet, ["type", "feed_leg"]), DeviceBulkAddComponentForm
):
    field_order = (
        "name_pattern",
        "label_pattern",
...
class PowerOutletBulkEditForm(
    form_from_model(
        PowerOutlet, ["label", "type", "feed_leg", "power_port", "description"]
    ),
    BootstrapMixin,
    AddRemoveTagsForm,
    BulkEditForm,
    ...
    pk = forms.ModelMultipleChoiceField(
        queryset=PowerOutlet.objects.all(), widget=forms.MultipleHiddenInput()
    )
...
class PowerOutletCSVForm(CSVModelForm):
    device = CSVModelChoiceField(queryset=Device.objects.all(), to_field_name="name")
    type = CSVChoiceField(
        choices=PowerOutletTypeChoices, required=False, help_text="Outlet type"
    )
...
class InterfaceFilterForm(DeviceComponentFilterForm):
    model = Interface
    type = forms.MultipleChoiceField(
        choices=InterfaceTypeChoices, required=False, widget=StaticSelect2Multiple()
    )
...
class InterfaceBulkEditForm(
    form_from_model(
        Interface, ["label", "type", "lag", "mac_address", "mtu", "description", "mode"]
    ),
    BootstrapMixin,
    AddRemoveTagsForm,
    BulkEditForm,
    ...
    def clean(self):
        ...
        if (
            self.cleaned_data["mode"] == InterfaceModeChoices.MODE_ACCESS
            and self.cleaned_data["tagged_vlans"]
        ):
            raise forms.ValidationError(
                {"mode": "An access interface cannot have tagged VLANs assigned."}
            )
...
class FrontPortFilterForm(DeviceComponentFilterForm):
    model = FrontPort
    type = forms.MultipleChoiceField(
        choices=PortTypeChoices, required=False, widget=StaticSelect2Multiple()
    )
    tag = TagFilterField(model)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if hasattr(self.instance, "device"):
            self.fields["rear_port"].queryset = self.fields[
                "rear_port"
            ].queryset.filter(device=self.instance.device)
...
class FrontPortBulkEditForm(
    ...
    pk = forms.ModelMultipleChoiceField(
        queryset=FrontPort.objects.all(), widget=forms.MultipleHiddenInput()
    )
...
class FrontPortCSVForm(CSVModelForm):
    ...
    type = CSVChoiceField(
        choices=PortTypeChoices, help_text="Physical medium classification"
    )
...
class RearPortBulkCreateForm(
    form_from_model(RearPort, ["type", "positions"]), DeviceBulkAddComponentForm
):
    field_order = (
        "name_pattern",
        "label_pattern",
...
class RearPortBulkEditForm(
    ...
    pk = forms.ModelMultipleChoiceField(
        queryset=RearPort.objects.all(), widget=forms.MultipleHiddenInput()
    )
...
class DeviceBayBulkEditForm(
    ...
    pk = forms.ModelMultipleChoiceField(
        queryset=DeviceBay.objects.all(), widget=forms.MultipleHiddenInput()
    )
...
class InventoryItemBulkCreateForm(
    form_from_model(
        InventoryItem, ["manufacturer", "part_id", "serial", "asset_tag", "discovered"]
    ),
    DeviceBulkAddComponentForm,
):
    field_order = (
...
class InventoryItemBulkEditForm(
    ...
    pk = forms.ModelMultipleChoiceField(
        queryset=InventoryItem.objects.all(), widget=forms.MultipleHiddenInput()
    )
    manufacturer = DynamicModelChoiceField(
        queryset=Manufacturer.objects.all(), required=False
    )
...
class CableCSVForm(CustomFieldModelCSVForm):
    ...
    side_a_device = CSVModelChoiceField(
        queryset=Device.objects.all(), to_field_name="name", help_text="Side A device"
    )
    ...
    side_b_device = CSVModelChoiceField(
        queryset=Device.objects.all(), to_field_name="name", help_text="Side B device"
    )
    ...
    # Cable attributes
    status = CSVChoiceField(
        choices=CableStatusChoices, required=False, help_text="Connection status"
    )
    type = CSVChoiceField(
    ...
    length_unit = CSVChoiceField(
        choices=CableLengthUnitChoices, required=False, help_text="Length unit"
    )
    ...
    def _clean_side(self, side):
        ...
        try:
            ...
            if termination_object.cable is not None:
                raise forms.ValidationError(
                    f"Side {side.upper()}: {device} {termination_object} is already connected"
                )
        except ObjectDoesNotExist:
            raise forms.ValidationError(
                f"{side.upper()} side termination not found: {device} {name}"
            )
...
class CableBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=Cable.objects.all(), widget=forms.MultipleHiddenInput
    )
    type = forms.ChoiceField(
    ...
    color = forms.CharField(
        max_length=6, required=False, widget=ColorSelect()  # RGB color code
    )
...
class CableFilterForm(BootstrapMixin, forms.Form):
    ...
    tenant = DynamicModelMultipleChoiceField(
        queryset=Tenant.objects.all(), to_field_name="slug", required=False
    )
    rack_id = DynamicModelMultipleChoiceField(
        queryset=Rack.objects.all(),
        required=False,
    ...
    def save(self, *args, **kwargs):
        ...
        if instance.pk:
            ...
            for i, member in enumerate(
                self.cleaned_data["members"], start=initial_position
            ):
        ...
        for form in self.forms:
            ...
            if vc_position:
                if vc_position in vc_position_list:
                    error_msg = "A virtual chassis member already exists in position {}.".format(
                        vc_position
                    )
...
class VirtualChassisBulkEditForm(
    BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm
):
    pk = forms.ModelMultipleChoiceField(
        queryset=VirtualChassis.objects.all(), widget=forms.MultipleHiddenInput()
    )
...
class PowerPanelBulkEditForm(
    ...
    pk = forms.ModelMultipleChoiceField(
        queryset=PowerPanel.objects.all(), widget=forms.MultipleHiddenInput
    )
    region = DynamicModelChoiceField(
        queryset=Region.objects.all(), required=False, initial_params={"sites": "$site"}
    )
...
class PowerFeedForm(BootstrapMixin, CustomFieldModelForm):
    ...
    power_panel = DynamicModelChoiceField(
        queryset=PowerPanel.objects.all(), query_params={"site_id": "$site"}
    )
    rack = DynamicModelChoiceField(
        queryset=Rack.objects.all(),
        required=False,
...
class PowerFeedCSVForm(CustomFieldModelCSVForm):
    ...
    status = CSVChoiceField(
        choices=PowerFeedStatusChoices, required=False, help_text="Operational status"
    )
    type = CSVChoiceField(
        choices=PowerFeedTypeChoices, required=False, help_text="Primary or redundant"
    )
    supply = CSVChoiceField(
        choices=PowerFeedSupplyChoices, required=False, help_text="Supply type (AC/DC)"
    )
    phase = CSVChoiceField(
        choices=PowerFeedPhaseChoices, required=False, help_text="Single or three-phase"
    )
    ...
    def __init__(self, data=None, *args, **kwargs):
        ...
        if data:
            ...
            params = {f"site__{self.fields['site'].to_field_name}": data.get("site")}
            self.fields["power_panel"].queryset = self.fields[
                "power_panel"
            ].queryset.filter(**params)
...
class PowerFeedBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=PowerFeed.objects.all(), widget=forms.MultipleHiddenInput
    )
    power_panel = DynamicModelChoiceField(
        queryset=PowerPanel.objects.all(), required=False
    )
    rack = DynamicModelChoiceField(
        queryset=Rack.objects.all(), required=False, display_field="display_name"
    )
    status = forms.ChoiceField(
...
class PowerFeedFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    status = forms.MultipleChoiceField(
        choices=PowerFeedStatusChoices, required=False, widget=StaticSelect2Multiple()
    )
    type = forms.ChoiceField(
        choices=add_blank_choice(PowerFeedTypeChoices),
        required=False,
