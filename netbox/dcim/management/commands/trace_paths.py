...
class Command(BaseCommand):
    help = (
        "Generate any missing cable paths among all cable termination objects in NetBox"
    )
    ...
    def draw_progress_bar(self, percentage):
        ...
        self.stdout.write(
            f"\r  [{'#' * bar_size}{' ' * (20-bar_size)}] {int(percentage)}%", ending=""
        )
    ...
    def handle(self, *model_names, **options):
        ...
        if options["force"]:
            ...
            if paths_count and not options["no_input"]:
                self.stdout.write(
                    self.style.ERROR(
                        "WARNING: Forcing recalculation of all cable paths."
                    )
                )
                self.stdout.write(
        ...
        for model in ENDPOINT_MODELS:
            ...
            if not origins_count:
                self.stdout.write(
                    f"Found no missing {model._meta.verbose_name} paths; skipping"
                )
                continue
            self.stdout.write(
                f"Retracing {origins_count} cabled {model._meta.verbose_name_plural}..."
            )
            i = 0
