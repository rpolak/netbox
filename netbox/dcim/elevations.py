...
class RackElevationSVG:
    ...
    def _setup_drawing(width, height):
        ...
        # add the stylesheet
        with open(
            "{}/css/rack_elevation.css".format(settings.STATICFILES_DIRS[0])
        ) as css_file:
    ...
    def _draw_device_front(self, drawing, device, start, end, text):
        ...
        if device.devicebay_count:
            name += " ({}/{})".format(
                device.get_children().count(), device.devicebay_count
            )
        ...
        link = drawing.add(
            drawing.a(
                href="{}{}".format(
                    self.base_url, reverse("dcim:device", kwargs={"pk": device.pk})
                ),
                target="_top",
                fill="black",
            )
        ...
        link.add(
            drawing.rect(start, end, style="fill: #{}".format(color), class_="slot")
        )
    ...
    def merge_elevations(self, face):
        ...
        for u in elevation:
            ...
            if (
                not u["device"]
                and o["device"]
                and o["device"].device_type.is_full_depth
            ):
                u["device"] = o["device"]
                u["height"] = 1
    ...
    def render(self, face, unit_width, unit_height, legend_width):
        ...
        for unit in self.merge_elevations(face):
            ...
            if (
                ...
                and device.pk in self.permitted_device_ids
            ):
                self._draw_device_front(
                    drawing, device, start_cordinates, end_cordinates, text_cordinates
                )
            elif (
                device
                and device.device_type.is_full_depth
                and device.pk in self.permitted_device_ids
            ):
                self._draw_device_rear(
                    drawing, device, start_cordinates, end_cordinates, text_cordinates
                )
            elif device:
                # Devices which the user does not have permission to view are rendered only as unavailable space
                drawing.add(
                    drawing.rect(start_cordinates, end_cordinates, class_="blocked")
                )
            else:
