...
class ClusterAddDevicesForm(BootstrapMixin, forms.Form):
    region = DynamicModelChoiceField(
        queryset=Region.objects.all(), required=False, null_option="None"
    )
    site = DynamicModelChoiceField(
        queryset=Site.objects.all(),
        required=False,
...
class VirtualMachineForm(BootstrapMixin, TenancyForm, CustomFieldModelForm):
    ...
    cluster = DynamicModelChoiceField(
        queryset=Cluster.objects.all(), query_params={"group_id": "$cluster_group"}
    )
...
class VirtualMachineFilterForm(
    BootstrapMixin, TenancyFilterForm, CustomFieldFilterForm
):
    model = VirtualMachine
    field_order = [
        "q",
...
class VMInterfaceBulkEditForm(BootstrapMixin, AddRemoveTagsForm, BulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=VMInterface.objects.all(), widget=forms.MultipleHiddenInput()
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "virtual_machine" in self.initial:
            parent_obj = VirtualMachine.objects.filter(
                pk=self.initial["virtual_machine"]
            ).first()
