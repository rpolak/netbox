...
class ProviderBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=Provider.objects.all(), widget=forms.MultipleHiddenInput
    )
    ...
    noc_contact = forms.CharField(
        required=False, widget=SmallTextarea, label="NOC contact"
    )
    admin_contact = forms.CharField(
        required=False, widget=SmallTextarea, label="Admin contact"
    )
...
class CircuitCSVForm(CustomFieldModelCSVForm):
    ...
    status = CSVChoiceField(
        choices=CircuitStatusChoices, required=False, help_text="Operational status"
    )
...
class CircuitBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    pk = forms.ModelMultipleChoiceField(
        queryset=Circuit.objects.all(), widget=forms.MultipleHiddenInput
    )
...
class CircuitFilterForm(BootstrapMixin, TenancyFilterForm, CustomFieldFilterForm):
    ...
    provider = DynamicModelMultipleChoiceField(
        queryset=Provider.objects.all(), to_field_name="slug", required=False
    )
    status = forms.MultipleChoiceField(
        choices=CircuitStatusChoices, required=False, widget=StaticSelect2Multiple()
    )
    region = DynamicModelMultipleChoiceField(
        queryset=Region.objects.all(), to_field_name="slug", required=False
    )
