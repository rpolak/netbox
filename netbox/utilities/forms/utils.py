...
def parse_alphanumeric_range(string):
    ...
    for dash_range in string.split(","):
        try:
            ...
            if (not (vals.isdigit() or vals.isalpha())) or (
                vals.isalpha() and not (vals.isupper() or vals.islower())
            ):
                return []
        except ValueError:
            begin, end = dash_range, dash_range
            ...
            # Range-based
                ...
                if not len(begin) == len(end) == 1:
                    raise forms.ValidationError(
                        'Range "{}" is invalid.'.format(dash_range)
                    )
...
def restrict_form_fields(form, user, action="view"):
    ...
    for field in form.fields.values():
        if hasattr(field, "queryset") and issubclass(
            field.queryset.__class__, RestrictedQuerySet
        ):
