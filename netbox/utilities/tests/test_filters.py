...
class TreeNodeMultipleChoiceFilterTest(TestCase):
    ...
    def setUp(self):
        ...
        self.site3 = Site.objects.create(
            region=None, name="Test Site 3", slug="test-site3"
        )
...
class BaseFilterSetTest(TestCase):
    ...
    def test_mac_address_filter(self):
        ...
        self.assertEqual(
            self.filters["macaddressfield__isw"].lookup_expr, "istartswith"
        )
        self.assertEqual(self.filters["macaddressfield__isw"].exclude, False)
        self.assertEqual(
            self.filters["macaddressfield__nisw"].lookup_expr, "istartswith"
        )
    ...
    def test_model_choice_filter(self):
        self.assertIsInstance(
            self.filters["modelchoicefield"], django_filters.ModelChoiceFilter
        )
    ...
    def test_model_multiple_choice_filter(self):
        ...
        self.assertEqual(
            self.filters["modelmultiplechoicefield__n"].lookup_expr, "exact"
        )
    ...
    def test_multi_value_char_filter(self):
        ...
        self.assertEqual(
            self.filters["multivaluecharfield__ic"].lookup_expr, "icontains"
        )
        self.assertEqual(self.filters["multivaluecharfield__ic"].exclude, False)
        self.assertEqual(
            self.filters["multivaluecharfield__nic"].lookup_expr, "icontains"
        )
        self.assertEqual(self.filters["multivaluecharfield__nic"].exclude, True)
        self.assertEqual(
            self.filters["multivaluecharfield__isw"].lookup_expr, "istartswith"
        )
        self.assertEqual(self.filters["multivaluecharfield__isw"].exclude, False)
        self.assertEqual(
            self.filters["multivaluecharfield__nisw"].lookup_expr, "istartswith"
        )
        self.assertEqual(self.filters["multivaluecharfield__nisw"].exclude, True)
        self.assertEqual(
            self.filters["multivaluecharfield__iew"].lookup_expr, "iendswith"
        )
        self.assertEqual(self.filters["multivaluecharfield__iew"].exclude, False)
        self.assertEqual(
            self.filters["multivaluecharfield__niew"].lookup_expr, "iendswith"
        )
    ...
    def test_multiple_choice_filter(self):
        self.assertIsInstance(
            self.filters["multiplechoicefield"], django_filters.MultipleChoiceFilter
        )
        ...
        self.assertEqual(
            self.filters["multiplechoicefield__ic"].lookup_expr, "icontains"
        )
        self.assertEqual(self.filters["multiplechoicefield__ic"].exclude, False)
        self.assertEqual(
            self.filters["multiplechoicefield__nic"].lookup_expr, "icontains"
        )
        self.assertEqual(self.filters["multiplechoicefield__nic"].exclude, True)
        self.assertEqual(
            self.filters["multiplechoicefield__isw"].lookup_expr, "istartswith"
        )
        self.assertEqual(self.filters["multiplechoicefield__isw"].exclude, False)
        self.assertEqual(
            self.filters["multiplechoicefield__nisw"].lookup_expr, "istartswith"
        )
        self.assertEqual(self.filters["multiplechoicefield__nisw"].exclude, True)
        self.assertEqual(
            self.filters["multiplechoicefield__iew"].lookup_expr, "iendswith"
        )
        self.assertEqual(self.filters["multiplechoicefield__iew"].exclude, False)
        self.assertEqual(
            self.filters["multiplechoicefield__niew"].lookup_expr, "iendswith"
        )
    ...
    def test_tree_node_multiple_choice_filter(self):
        self.assertIsInstance(
            self.filters["treeforeignkeyfield"], TreeNodeMultipleChoiceFilter
        )
...
class DynamicFilterLookupExpressionTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        interfaces = (
            ...
            Interface(
                device=devices[0], name="Interface 2", mac_address="aa-00-00-00-00-01"
            ),
            Interface(
                device=devices[1], name="Interface 3", mac_address="00-00-00-00-00-02"
            ),
            Interface(
                device=devices[1], name="Interface 4", mac_address="bb-00-00-00-00-02"
            ),
            Interface(
                device=devices[2], name="Interface 5", mac_address="00-00-00-00-00-03"
            ),
            Interface(
                device=devices[2], name="Interface 6", mac_address="cc-00-00-00-00-03"
            ),
        )
