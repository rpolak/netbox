...
class ViewTestCases:
    ...
    class CreateObjectViewTestCase(ModelViewTestCase):
        ...
        def test_create_object_with_constrained_permission(self):
            ...
            obj_perm = ObjectPermission(
                constraints={"pk": 0}, actions=["add"]  # Dummy permission to deny all
            )
            obj_perm.save()
            obj_perm.users.add(self.user)
            obj_perm.object_types.add(ContentType.objects.get_for_model(self.model))
    ...
    class EditObjectViewTestCase(ModelViewTestCase):
        ...
        def test_edit_object_with_constrained_permission(self):
            ...
            # Assign constrained permission
            obj_perm = ObjectPermission(
                constraints={"pk": instance1.pk}, actions=["change"]
            )
            obj_perm.save()
            obj_perm.users.add(self.user)
            obj_perm.object_types.add(ContentType.objects.get_for_model(self.model))
            ...
            self.assertHttpStatus(self.client.post(**request), 302)
            self.assertInstanceEqual(
                self._get_queryset().get(pk=instance1.pk), self.form_data
            )
    ...
    class DeleteObjectViewTestCase(ModelViewTestCase):
        ...
        def test_delete_object_with_constrained_permission(self):
            ...
            obj_perm = ObjectPermission(
                constraints={"pk": instance1.pk}, actions=["delete"]
            )
            obj_perm.save()
            obj_perm.users.add(self.user)
            obj_perm.object_types.add(ContentType.objects.get_for_model(self.model))
            ...
            # Try GET with a permitted object
            self.assertHttpStatus(
                self.client.get(self._get_url("delete", instance1)), 200
            )

            # Try GET with a non-permitted object
            self.assertHttpStatus(
                self.client.get(self._get_url("delete", instance2)), 404
            )
    ...
    class CreateMultipleObjectsViewTestCase(ModelViewTestCase):
        ...
        def test_create_multiple_objects_with_permission(self):
            ...
            self.assertEqual(
                initial_count + self.bulk_create_count, self._get_queryset().count()
            )
            for instance in self._get_queryset().order_by("-pk")[
                : self.bulk_create_count
            ]:
        ...
        def test_create_multiple_objects_with_constrained_permission(self):
            ...
            # Assign constrained permission
            obj_perm = ObjectPermission(
                actions=["add"], constraints={"pk": 0}  # Dummy constraint to deny all
            )
            obj_perm.save()
            obj_perm.users.add(self.user)
            obj_perm.object_types.add(ContentType.objects.get_for_model(self.model))
    ...
    class BulkImportObjectsViewTestCase(ModelViewTestCase):
        ...
        def test_bulk_import_objects_with_permission(self):
            ...
            self.assertEqual(
                self._get_queryset().count(), initial_count + len(self.csv_data) - 1
            )
    ...
    class BulkEditObjectsViewTestCase(ModelViewTestCase):
        ...
        def test_bulk_edit_objects_without_permission(self):
            ...
            with disable_warnings("django.request"):
                self.assertHttpStatus(
                    self.client.post(self._get_url("bulk_edit"), data), 403
                )
        ...
        def test_bulk_edit_objects_with_constrained_permission(self):
            ...
            # Assign constrained permission
            obj_perm = ObjectPermission(
                constraints={attr_name: value}, actions=["change"]
            )
            obj_perm.save()
            obj_perm.users.add(self.user)
            obj_perm.object_types.add(ContentType.objects.get_for_model(self.model))
    ...
    class BulkDeleteObjectsViewTestCase(ModelViewTestCase):
        ...
        def test_bulk_delete_objects_without_permission(self):
            ...
            with disable_warnings("django.request"):
                self.assertHttpStatus(
                    self.client.get(self._get_url("bulk_delete")), 403
                )

            # Try POST without permission
            with disable_warnings("django.request"):
                self.assertHttpStatus(
                    self.client.post(self._get_url("bulk_delete"), data), 403
                )
    ...
    class BulkRenameObjectsViewTestCase(ModelViewTestCase):
        ...
        def test_bulk_rename_objects_without_permission(self):
            ...
            with disable_warnings("django.request"):
                self.assertHttpStatus(
                    self.client.get(self._get_url("bulk_rename")), 403
                )

            # Try POST without permission
            with disable_warnings("django.request"):
                self.assertHttpStatus(
                    self.client.post(self._get_url("bulk_rename"), data), 403
                )
        ...
        def test_bulk_rename_objects_with_constrained_permission(self):
            ...
            # Assign constrained permission
            obj_perm = ObjectPermission(
                constraints={"name__regex": "[^X]$"}, actions=["change"]
            )
            obj_perm.save()
            obj_perm.users.add(self.user)
            obj_perm.object_types.add(ContentType.objects.get_for_model(self.model))
