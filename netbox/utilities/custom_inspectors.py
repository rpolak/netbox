            ...
            if properties:
                if type(serializer) not in self.writable_serializers:
                    ...
                    if meta_class:
                        ...
                        writable_meta = type(
                            "Meta", (meta_class,), {"ref_name": ref_name}
                        )
    ...
    def field_to_swagger_object(
        ...
        if isinstance(field, SerializedPKRelatedField):
            return self.probe_field_inspectors(
                field.serializer(), ChildSwaggerType, use_references
            )
    ...
    def field_to_swagger_object(
        ...
        if isinstance(field, ChoiceField):
            ...
            if set([None] + choice_value) == {None, True, False}:
                ...
                if all(
                    type(x) == bool for x in [c for c in choice_value if c is not None]
                ):
            ...
            if all(type(x) == int for x in [c for c in choice_value if c is not None]):
                ...
                value_schema = openapi.Schema(
                    type=openapi.TYPE_INTEGER, enum=choice_value
                )
