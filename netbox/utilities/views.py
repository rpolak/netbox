...
class ObjectListView(ObjectPermissionRequiredMixin, View):
    ...
    def get(self, request):
        ...
        if request.GET.get("export"):
            ...
            queryset = (
                CustomFieldQueryset(self.queryset, custom_fields)
                if custom_fields
                else self.queryset
            )
            try:
        ...
        if form.is_valid():
            if "set" in request.POST:
                request.user.config.set(
                    preference_name, form.cleaned_data["columns"], commit=True
                )
            ...
            try:
                with transaction.atomic():
                    ...
                    if self.queryset.filter(
                        pk__in=[obj.pk for obj in new_objs]
                    ).count() != len(new_objs):
        ...
        if form.is_valid():
                    ...
                    with transaction.atomic():
                        ...
                        for (
                            ...
                            if model.objects.filter(
                                pk__in=related_obj_pks
                            ).count() != len(related_obj_pks):
                                raise ObjectDoesNotExist
            ...
            if not model_form.errors:
                ...
                messages.success(
                    request,
                    mark_safe(
                        'Imported object: <a href="{}">{}</a>'.format(
...
class BulkImportView(GetReturnURLMixin, ObjectPermissionRequiredMixin, View):
    ...
    def _import_form(self, *args, **kwargs):
        class ImportForm(BootstrapMixin, Form):
            csv = CSVDataField(
                from_form=self.model_form, widget=Textarea(attrs=self.widget_attrs)
            )
...
class BulkEditView(GetReturnURLMixin, ObjectPermissionRequiredMixin, View):
    ...
    def post(self, request, **kwargs):
        ...
        if request.POST.get("_all") and self.filterset is not None:
            pk_list = [
                obj.pk
                for obj in self.filterset(request.GET, self.queryset.only("pk")).qs
            ]
        else:
        ...
        if "_apply" in request.POST:
            ...
            if form.is_valid():
                logger.debug("Form validation was successful")
                custom_fields = (
                    form.custom_fields if hasattr(form, "custom_fields") else []
                )
                standard_fields = [
                    field
                ...
                try:
                    ...
                    if updated_objects:
                        msg = "Updated {} {}".format(
                            len(updated_objects), model._meta.verbose_name_plural
                        )
                        logger.info(msg)
                    ...
                    msg = (
                        "Object update failed due to object-level permissions violation"
                    )
                    logger.debug(msg)
                    form.add_error(None, msg)
                ...
                try:
                    with transaction.atomic():
                        ...
                        if "_apply" in request.POST:
                            ...
                            if self.queryset.filter(pk__in=renamed_pks).count() != len(
                                selected_objects
                            ):
                                raise ObjectDoesNotExist
...
class BulkComponentCreateView(GetReturnURLMixin, ObjectPermissionRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if request.POST.get("_all") and self.filterset is not None:
            pk_list = [
                obj.pk
                for obj in self.filterset(
                    request.GET, self.parent_model.objects.only("pk")
                ).qs
            ]
        else:
            pk_list = [int(pk) for pk in request.POST.getlist("pk")]
            ...
            if form.is_valid():
                ...
                try:
                    with transaction.atomic():
                        ...
                        for obj in data["pk"]:
                            ...
                            for i, name in enumerate(names):
                                ...
                                if component_form.is_valid():
                                    ...
                                    logger.debug(
                                        f"Created {instance} on {instance.parent}"
                                    )
                        ...
                        # Enforce object-level permissions
                        if self.queryset.filter(
                            pk__in=[obj.pk for obj in new_components]
