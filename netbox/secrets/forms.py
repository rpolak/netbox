...
class SecretCSVForm(CustomFieldModelCSVForm):
    device = CSVModelChoiceField(
        queryset=Device.objects.all(), to_field_name="name", help_text="Assigned device"
    )
...
class SecretBulkEditForm(BootstrapMixin, AddRemoveTagsForm, CustomFieldBulkEditForm):
    ...
    role = DynamicModelChoiceField(queryset=SecretRole.objects.all(), required=False)
...
class SecretFilterForm(BootstrapMixin, CustomFieldFilterForm):
    ...
    role = DynamicModelMultipleChoiceField(
        queryset=SecretRole.objects.all(), to_field_name="slug", required=False
    )
