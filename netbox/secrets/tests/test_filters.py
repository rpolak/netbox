...
class SecretTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        secrets = (
            Secret(
                assigned_object=devices[0],
                role=roles[0],
                name="Secret 1",
                plaintext="SECRET DATA",
            ),
            Secret(
                assigned_object=devices[1],
                role=roles[1],
                name="Secret 2",
                plaintext="SECRET DATA",
            ),
            Secret(
                assigned_object=devices[2],
                role=roles[2],
                name="Secret 3",
                plaintext="SECRET DATA",
            ),
            Secret(
                assigned_object=virtual_machines[0],
                role=roles[0],
                name="Secret 4",
                plaintext="SECRET DATA",
            ),
            Secret(
                assigned_object=virtual_machines[1],
                role=roles[1],
                name="Secret 5",
                plaintext="SECRET DATA",
            ),
            Secret(
                assigned_object=virtual_machines[2],
                role=roles[2],
                name="Secret 6",
                plaintext="SECRET DATA",
            ),
    ...
    def test_virtual_machine(self):
        ...
        params = {
            "virtual_machine_id": [virtual_machines[0].pk, virtual_machines[1].pk]
        }
        self.assertEqual(self.filterset(params, self.queryset).qs.count(), 2)
        params = {
            "virtual_machine": [virtual_machines[0].name, virtual_machines[1].name]
        }
        self.assertEqual(self.filterset(params, self.queryset).qs.count(), 2)
