...
class UserKeyTestCase(TestCase):
    def setUp(self):
        ...
        for username in ["alice", "bob"]:
            ...
            self.TEST_KEYS["{}_public".format(username)] = key.publickey().exportKey(
                "PEM"
            )
    ...
    def test_01_fill(self):
        ...
        self.assertFalse(
            alice_uk.is_filled(),
            "UserKey with empty public_key is_filled() did not return False",
        )
    ...
    def test_02_activate(self):
        ...
        alice_uk = UserKey(
            ...
            public_key=self.TEST_KEYS["alice_public"],
        )
        self.assertFalse(
            alice_uk.is_active(), "Inactive UserKey is_active() did not return False"
        )
        alice_uk.activate(master_key)
        self.assertTrue(
            alice_uk.is_active(), "ActiveUserKey is_active() did not return True"
        )
    ...
    def test_03_key_sizes(self):
        ...
        rsa = RSA.generate(
            4096 + 256
        )  # Max size is 4096 (enforced by master_key_cipher field size)
    ...
    def test_04_master_key_retrieval(self):
        ...
        self.assertEqual(
            master_key,
            retrieved_master_key,
            "Master key retrieval failed with correct private key",
        )
    ...
    def test_05_invalid_private_key(self):
        ...
        secret_key_cipher = encrypt_master_key(
            secret_key, self.TEST_KEYS["alice_public"]
        )
        try:
            decrypted_secret_key = decrypt_master_key(
                secret_key_cipher, self.TEST_KEYS["bob_private"]
            )
            self.fail(
                "Decrypting secret key from Alice's UserKey using Bob's private key did not fail"
            )
        except ValueError:
            pass
...
class SecretTestCase(TestCase):
    ...
    def test_01_encrypt_decrypt(self):
        ...
        self.assertGreaterEqual(
            len(salt), 12, "Hash salt is too short ({} chars)".format(len(salt))
        )
        ...
        self.assertFalse(
            s.validate("Invalid plaintext"), "Invalid plaintext validated against hash"
        )
