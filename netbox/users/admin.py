...
class ObjectPermissionInline(admin.TabularInline):
    ...
    def get_queryset(self, request):
        return (
            ...
            .prefetch_related("objectpermission__object_types")
            .nocache()
        )
...
class ObjectPermissionForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["users"].queryset = self.fields["users"].queryset.order_by(
            "username"
        )
    ...
    def clean(self):
        ...
        for action in ["view", "add", "change", "delete"]:
            if (
                self.cleaned_data[f"can_{action}"]
                and action not in self.cleaned_data["actions"]
            ):
...
class ActionListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        options = set()
        for action_list in ObjectPermission.objects.values_list(
            "actions", flat=True
        ).distinct():
...
class ObjectTypeListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        content_types = ContentType.objects.filter(pk__in=object_types).order_by(
            "app_label", "model"
        )
