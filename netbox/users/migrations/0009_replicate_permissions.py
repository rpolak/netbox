    ...
    for perm in Permission.objects.select_related("content_type"):
        ...
        if perm.group_set.exists() or perm.user_set.exists():
            ...
            if perm.codename == "view_secret":
                for secretrole in SecretRole.objects.prefetch_related(
                    "users", "groups"
                ):
                    obj_perm = ObjectPermission(
                        name=f"{perm.content_type.app_label}.{perm.codename} ({secretrole.name})"[
                            :100
                        ],
                        actions=[action],
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(
            code=replicate_permissions, reverse_code=migrations.RunPython.noop
        )
    ]
