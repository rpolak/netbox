...
class TagListView(ObjectListView):
    queryset = Tag.objects.annotate(items=get_subquery(TaggedItem, "tag"))
...
class ConfigContextView(ObjectView):
    ...
    def get(self, request, pk):
        ...
        self.queryset = self.queryset.prefetch_related(
            ...
            Prefetch(
                "cluster_groups", queryset=ClusterGroup.objects.restrict(request.user)
            ),
            Prefetch("tenants", queryset=Tenant.objects.restrict(request.user)),
            Prefetch(
                "tenant_groups", queryset=TenantGroup.objects.restrict(request.user)
            ),
        )
        ...
        if request.GET.get("format") in ["json", "yaml"]:
            ...
            if request.user.is_authenticated:
                request.user.config.set(
                    "extras.configcontext.format", format, commit=True
                )
...
class ObjectChangeView(ObjectView):
    ...
    def get(self, request, pk):
        ...
        related_changes = (
            ...
            .filter(request_id=objectchange.request_id)
            .exclude(pk=objectchange.pk)
        )
        ...
        next_change = (
            objectchanges.filter(time__gt=objectchange.time).order_by("time").first()
        )
        prev_change = (
            objectchanges.filter(time__lt=objectchange.time).order_by("-time").first()
        )
...
class ReportView(ContentTypePermissionRequiredMixin, View):
    ...
    def post(self, request, module, name):
        ...
        if not Worker.count(get_connection("default")):
            messages.error(
                request, "Unable to run report: RQ worker process not running."
            )
            return render(
                request,
                "extras/report.html",
...
class ReportResultView(ContentTypePermissionRequiredMixin, View):
    ...
    def get(self, request, job_result_pk):
        ...
        jobresult = get_object_or_404(
            JobResult.objects.all(), pk=job_result_pk, obj_type=report_content_type
        )
...
class ScriptView(ContentTypePermissionRequiredMixin, GetScriptMixin, View):
    ...
    def get(self, request, module, name):
        ...
        script.result = (
            ...
            .exclude(status__in=JobResultStatusChoices.TERMINAL_STATE_CHOICES)
            .first()
        )
    ...
    def post(self, request, module, name):
        ...
        if not Worker.count(get_connection("default")):
            messages.error(
                request, "Unable to run script: RQ worker process not running."
            )
