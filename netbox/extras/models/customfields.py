...
class CustomField(models.Model):
    ...
    def to_form_field(
        ...
        field.model = self
        field.label = (
            self.label if self.label else self.name.replace("_", " ").capitalize()
        )
        if self.description:
...
class CustomFieldValue(models.Model):
    field = models.ForeignKey(
        to="extras.CustomField", on_delete=models.CASCADE, related_name="values"
    )
    obj_type = models.ForeignKey(
        to=ContentType, on_delete=models.PROTECT, related_name="+"
    )
    obj_id = models.PositiveIntegerField()
    obj = GenericForeignKey(ct_field="obj_type", fk_field="obj_id")
...
class CustomFieldChoice(models.Model):
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        CustomFieldValue.objects.filter(
            field__type=CustomFieldTypeChoices.TYPE_SELECT, serialized_value=str(pk)
        ).delete()
