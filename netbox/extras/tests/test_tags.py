...
class TaggedItemTest(APITestCase):
    ...
    def test_create_tagged_item(self):
        ...
        data = {"name": "Test Site", "slug": "test-site", "tags": [t.pk for t in tags]}
        ...
        self.assertListEqual(
            sorted([t["id"] for t in response.data["tags"]]), sorted(data["tags"])
        )
        site = Site.objects.get(pk=response.data["id"])
        self.assertListEqual(
            sorted([t.name for t in site.tags.all()]), sorted(["Foo", "Bar", "Baz"])
        )
    ...
    def test_update_tagged_item(self):
        ...
        site = Site.objects.get(pk=response.data["id"])
        self.assertListEqual(
            sorted([t.name for t in site.tags.all()]), sorted(["Foo", "Bar", "New Tag"])
        )
