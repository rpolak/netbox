...
def process_webhook(webhook, data, model_name, event, timestamp, username, request_id):
    ...
    try:
        ...
        logger.error(
            "Error rendering request body for webhook {}: {}".format(webhook, e)
        )
        raise e
    ...
    if 200 <= response.status_code <= 299:
        logger.info(
            "Request succeeded; response status {}".format(response.status_code)
        )
        ...
        logger.warning(
            "Request failed; response status {}: {}".format(
