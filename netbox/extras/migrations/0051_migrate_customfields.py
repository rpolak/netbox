...
def migrate_customfieldchoices(apps, schema_editor):
    ...
    for cf in CustomField.objects.filter(type="select"):
        cf.choices = [
            cfc.value
            for cfc in CustomFieldChoice.objects.filter(field=cf).order_by(
                "weight", "value"
            )
        ]
        cf.save()
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(code=migrate_customfield_defaults),
        migrations.RunPython(code=migrate_customfieldchoices),
        migrations.RunPython(code=migrate_customfieldvalues),
        migrations.RunPython(code=fix_filter_logic_values),
    ]
