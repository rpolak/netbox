        ...
        if names:
            ...
            for name in names:
                try:
                    ...
                    raise CommandError(
                        "Invalid format: {}. Models must be specified in the form app_label.ModelName.".format(
                            name
                        )
                    )
                try:
                ...
                try:
                    ...
                    raise CommandError(
                        "Unknown model: {}.{}".format(app_label, model_name)
                    )
                fields = [
                    field
                ...
                if not fields:
                    raise CommandError(
                        "Invalid model: {}.{} does not employ natural ordering".format(
        ...
        for model, fields in models:
            for field in fields:
                ...
                if options["verbosity"]:
                    self.stdout.write(
                        "{}.{} ({})... ".format(
                            model._meta.label, field.target_field, field.name
                        ),
                ...
                queryset = (
                    model.objects.values_list(target_field, flat=True)
                ...
                for value in queryset:
                    ...
                    if options["verbosity"] >= 2:
                        self.stdout.write(
                            "  {} -> {}".format(value, naturalized_value), ending=""
                        )
                        self.stdout.flush()
